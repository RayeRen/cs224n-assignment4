...
def read_write_dataset(dataset, tier, prefix):
    ...
    with open(os.path.join(prefix, tier + '.context'), 'w', encoding='utf-8') as context_file, \
            open(os.path.join(prefix, tier + '.question'), 'w', encoding='utf-8') as question_file, \
            open(os.path.join(prefix, tier + '.answer'), 'w', encoding='utf-8') as text_file, \
            open(os.path.join(prefix, tier + '.span'), 'w', encoding='utf-8') as span_file:
            ...
            for pid in range(len(article_paragraphs)):
                ...
                for qid in range(len(qas)):
                    ...
                    for ans_id in num_answers:
                        ...
                        last_word_answer = len(text_tokens[-1])  # add one to get the first charf
...
def split_tier(prefix, train_percentage=0.9, shuffle=False):
    ...
    with open(context_filename, encoding='utf-8') as current_file:
